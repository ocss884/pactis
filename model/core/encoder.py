...
class PerceiverEncoder(nn.Module, FromConfig):
    def __init__(
        self,
...
class PerceiverDecoder(nn.Module, FromConfig):
    def __init__(self, 
                num_latents: int,
                num_latent_dim: int,
                num_heads: int,
...
class PerceiverIO(Sequential, FromConfig):
    ...
    def from_config(cls, encoder_config: PerceiverEncoderConfig, decoder_config: PerceiverDecoder, **kwargs):
        encoder = PerceiverEncoder.from_config(**encoder_config.dict, **kwargs)
        decoder = PerceiverDecoder.from_config(**decoder_config.dict, **kwargs)
